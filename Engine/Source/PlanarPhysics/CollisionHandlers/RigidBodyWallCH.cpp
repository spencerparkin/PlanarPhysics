#include "PlanarPhysics/CollisionHandlers/RigidBodyWallCH.h"
...
/*virtual*/ bool RigidBodyWallCH::HandleCollision(PlanarObject* objectA, PlanarObject* objectB)
{
	...
	for(const PlanarObject::Contact& contact : contactArray)
	{
		...
		{
			double coefficientOfRestitution = body->GetBounceFactor() * wall->GetBounceFactor();
			double j = -(1.0 + coefficientOfRestitution) * relativeVelocity / (1.0 / body->mass - (r ^ contact.normal) * (r ^ contact.normal) / body->inertia);
